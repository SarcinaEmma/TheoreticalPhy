...
\end{array}
\]
\footnotetext{Neutrinos admit only global $U(1)$ symmetry, and their masses are of the order $m_{\nu} \approx 10^{-6} \,m_e \le 1$eV.}
...
\begin{comment} %wrong, stated better later
...
\begin{center}
\input{img/hel_chir_UR_part.tex}
\end{center}
...
\begin{center}
\input{img/hel_chir_UR_antipart.tex}
\end{center}
\end{comment}
...
\begin{center}
\input{img/ee-ff_com_hel_theta_zero.tex}
\end{center}

\item $\abs{\mathM_{LL}}^2$ is zero when $\theta = \pm\pi$

\begin{center}
\input{img/ee-ff_com_hel_theta_pi.tex}
\end{center}
...
\begin{center}
\input{img/ee-ff_crossing_symm.tex}
\end{center}
...
\textsf{See Peskin sec 5.5; Schwartz sec 13.5}\\
...
\textsf{See Peskin sec. 5.5; Schwartz sec. 13.5.3; Mandl sec. 8.6}\\
...
We will express the cross section in terms of $\omega_k$ and $\theta$. We can find $\omega_{k'}$, the energy of the final photon, using the following trick:
\begin{equation}\label{eqn:QED-compton-on-shell}\begin{aligned}
m^2&=(p')^2=(p+k-k')^2=p^2+2p\cdot(k-k')-2k\cdot k'\\
&=m^2+2m(\omega_k-\omega_{k'})-2\omega_k\omega_{k'}(1-\cos\theta)
\end{aligned}\end{equation}
from which we can obtain Compton's formula for the shift in the photon wavelength:
...
\begin{equation}\label{eqn:QED-compton-energies-lab}\displaystyle
\omega_{k'}=\frac{\omega_k}{1+\frac{\omega_{k}}m(1-\cos\theta)}
\end{equation}

The unpolarized amplitude in the Lab frame is
\begin{align*}
|\overline{\mathcal M}|_{\textup{LAB}}^2&=2q^4\left[\left(\frac{\omega_{k'}}{\omega_k}+\frac{\omega_k}{\omega_{k'}}\right)+2m\left(\frac1{\omega_k}-\frac1{\omega_{k'}}\right)+m^2\left(\frac1{\omega_k}-\frac1{\omega_{k'}}\right)^2\right]\\
&=2q^4\left[\left(\frac{\omega_{k'}}{\omega_k}+\frac{\omega_k}{\omega_{k'}}\right)-\sin^2\theta\right]
\end{align*}

The covariant flux factor reads
\[I_{\textup{LAB}}=[(p\cdot k)^2-m_e^2m_\gamma^2]^{1/2}=|p\cdot k|=m\omega_k\]
while the 2-body phase space reads (notice that $\vert{\hat{\vec {k'}}}\vert=\omega_{k'}$)
\begin{align*}
\int\de\Phi_{(2)}&=\int \frac{\de^3k'}{(2\pi)^32\omega_{k'}}\frac{\de^3p'}{(2\pi)^32E'}(2\pi)^4\delta^4(k'+p'-k-p)
=\int\frac{|\vec{k'}|^2\de|\vec{k'}|\de\Omega}{(2\pi)^2}\frac{1}{4\omega_{k'}E'}\delta(\omega_{k'}+E'-\omega_k-m)\\
&=\int\frac{|\vec{k'}|^2\de|\vec{k'}|\de\Omega}{16\pi^2\omega_{k'}E'}\frac{\delta(\omega_{k'}-|{\vec k'}|)}{\displaystyle\left|\frac{\partial(\omega_{k'}+E'-\omega_k-m)}{\partial|\vec{k'}|}\right|}_{\omega_{k'}=|{\vec k'}|}
=\int\de\Omega\frac{\omega_{k'}}{16\pi^2E'}\left|\frac{\partial(\omega_{k'}+E')}{\partial|\vec{k'}|}\right|^{-1}_{\omega_{k'}=|{\vec k'}|}
\end{align*}
where 
\[E'=\left(m^2+(\vec k-\vec k')^2\right)^{1/2}
=\left[m^2+\omega_k^2+\omega_{k'}^2-2\omega_{k'}\omega_k\cos\theta\right]^{1/2}\]
\[\left|\frac{\partial E'}{\partial|\vec{k'}|}\right|_{\omega_{k'}=|\vec{k'}|}=\frac{\omega_{k'}-\omega_k\cos\theta}{E'}\]
and

\[\left|\frac{\partial(\omega_{k'}+E')}{\partial|\vec{k'}|}\right|_{\omega_{k'}=|\hat{\vec k'}|}
=\left|1+\frac{\omega_{k'}-\omega_k\cos\theta}{E'}\right|
=\left|\frac{E'+\omega_{k'}-\omega_k\cos\theta}{E'}\right|
=\left|\frac{m+\omega_k(1-\cos\theta)}{E'}\right|
=\frac{m\omega_k}{E'\omega_{k'}}
\]
where in the last step we used eq.~\eqref{eqn:QED-compton-on-shell}.
...
=\frac1{64\pi^2}\frac{\omega_{k'}}{I_{\textup{LAB}}E'}\left|\frac{\partial(\omega_{k'}+E')}{\partial|\vec{k'}|}\right|^{-1}_{\omega_{k'}=|{\vec k'}|}{|\overline{\mathcal M}|}_{\textup{LAB}}^2\\
...
\textsf{See Peskin sec. 5.5; Schwartz sec. 13.5.4}\\
...
\begin{align*}
E&=\sqrt{\vec k^2+m^2}\overset{m\approx0}{\approx}|\vec k|=\omega_k\\
E'&=\sqrt{\vec k'^2+m^2}\overset{m\approx0}{\approx}|\vec k'|=\omega_{k'}
\end{align*}
\begin{align*}
p\cdot k\,&=E\omega_k+|\vec k|^2=\omega_k(E+\omega_k)\overset{m\approx0}{\approx}2\omega_k^2\\
p\cdot p'&=E\,E'-\vec k\vec k'=E\,E'-|\vec k||\vec k'|\cos\theta=E\,E'-\omega_k\omega_{k'}\cos\theta\overset{m\approx0}{\approx}\omega_k\omega_{k'}(1-\cos\theta)\\
p\cdot k'&=E\,\omega_{k'}+\vec k\vec k'=E\,\omega_{k'}+|\vec k||\vec k'|\cos\theta=\omega_{k'}(E+\omega_k\cos\theta)\overset{m\approx0}{\approx}\omega_k\omega_{k'}(1+\cos\theta)
\end{align*}
We also have
\begin{align*}
s&=(p+k)^2=m^2+2p\cdot k=m^2+2\omega_k(E+\omega_k)\overset{m\approx0}{\approx}4\omega_k^2 \quad\rightarrow\quad \omega_k\overset{m\approx0}{\approx}\frac{\sqrt s}{2}\\
s&=(p'+k')^2=m^2+2p'\cdot k'=m^2+2\omega_{k'}(E'+\omega_k')\overset{m\approx0}{\approx}4\omega_{k'}^2 \quad\rightarrow\quad \omega_{k'}\overset{m\approx0}{\approx}\frac{\sqrt s}{2}
\end{align*}

Plugging these values into eq.~\eqref{eqn:Compton-unpolarized-squared} for c.o.m. frame with $E\gg m$ we get 
\begin{align*}
...
\end{align*}
Notice that the term $p\cdot k/p\cdot k'$ becomes divergent when the electron is emitted in the backward direction ($\theta\approx\pi$), while other terms are all of $\mathcal O(1)$ or smaller.
...
\end{align*}
\[ p\cdot k'=\omega_{k'}(E+\omega_k\cos\theta)\overset{m\approx0}{\approx}\omega_{k'}\left(\omega_k+\frac{m^2}{2\omega_k}+\omega_k\cos\theta\right)
=\omega_{k}^2\left(1+\cos\theta+\frac{m^2}{2\omega_k^2}\right)\]
\begin{align*}
{|\overline{\mathcal M}|}_{\textup{CM}}^2
&\approx2q^4\left(\frac{p\cdot k'}{p\cdot k}+\frac{p\cdot k}{p\cdot k'}\right)
\approx2q^4\left(\frac{1+\cos\theta}{2}+\frac{2}{\displaystyle1+\cos\theta+\frac{m^2}{2\omega_k^2}}\right)
\end{align*}
...
&\approx\frac{q^4}{32\pi^2s}\left(\frac{1+\cos\theta}{2}+\frac{2}{\displaystyle1+\cos\theta+\frac{m^2}{2\omega_k^2}}\right)\\
&\approx\frac{\alpha^2}{2s}\left(\frac{1+\cos\theta}{2}+\frac{2}{\displaystyle1+\cos\theta+\frac{m^2}{2\omega_k^2}}\right)
\end{align*}
or
\[{\left(\frac{\de\bar\sigma}{\de\cos\theta}\right)}_{\textup{CM}}
\approx\frac{\pi\alpha^2}{s}\left(\frac{1+\cos\theta}{2}+\frac{2}{\displaystyle1+\cos\theta+\frac{m^2}{2\omega_k^2}}\right)\]
...
The total Compton scattering cross section then reads:
\begin{align*}
\bar\sigma_{\textup{total}}=\int_{-1}^1\de(\cos\theta){\left(\frac{\de\bar\sigma}{\de(\cos\theta)}\right)}_{\textup{CM}}
&\approx 
\frac{\pi\alpha^2}{s}\int_{-1}^1\de(\cos\theta)\left(\frac{1+\cos\theta}{2}+\frac{2}{\displaystyle1+\cos\theta+\frac{m^2}{2\omega_k^2}}\right)\\
...
Find the total cross section of the annihilation process $e^+e^-\rightarrow2\gamma$ (\emph{pair annihilation into photons}. \\

\textit{Solution:}\\
See Peskin, pag. 168-169.
\end{exercise}
...
\textsf{See Schwartz sec. 13.4; Mandl sec. 8.7}\\
...
\[\frac{\de\sigma}{\de\Omega}=\frac{m_e^2e^4}{\displaystyle 4|\vec p_i|^4\sin^4\frac\theta2}\]
...
We study the process at relativistic level in the CM frame for the proton.
...
\quad\rightarrow\quad
\omega_e=\omega_e'+\frac{|\vec{p_{p}}'|^2}{2M}+o\left(|\vec{p_{p}}'|^{3}\right)
\quad\rightarrow\quad
...
In order to describe this process, we need at least two-vertices diagrams. Therefore the lowest order contribution to the process is given by the two-vertices $S$ matrix element
\begin{align*}
