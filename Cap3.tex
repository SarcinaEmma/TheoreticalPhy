...
\todo{Nei tuoi schemi ci sono delle sommatorie e altre cose nel diagramma di Feynman che non ho idea di come inserire}
...
\todo{Nei tuoi schemi in mezzo alle 4 linee continue ci sono delle sommatorie. Non ho idea di come metterle, sorry}
...
\begin{tikzpicture}[baseline=($0.5*(nu1)+0.5*(nu2)$)]
  \begin{feynman}[large]
    ...
    \vertex [label={[label distance=0.12cm]180:$\mu$}] at ([shift={(0:2.5)}]nu2) (mu2);
        \vertex [label={[label distance=0.1cm]2:$\nu$}, below=2cm of nu2] (nu1);
    \vertex [label={[label distance=0.12cm]180:$\mu$}] at ([shift={(0:2.5)}]nu1) (mu1);
    
    \diagram*{   
      (nu1) -- [fermion, half left, looseness=0.8, momentum'={[arrow shorten=0.35]\(p'\)}](mu1)
      		-- [fermion, half left, looseness=0.8, reversed momentum={[arrow shorten=0.35]\(q'\)}](nu1),
      (nu2) -- [fermion, half left, looseness=0.8, momentum={[arrow shorten=0.35]\(p\)}](mu2)
      		-- [fermion, half left, looseness=0.8, reversed momentum'={[arrow shorten=0.35]\(q\)}](nu2),
...
\todo{Se vuoi cambia il nome del file}
...
\draw    (231.3,109.43) .. controls (264.62,77.09) and (261.44,170.56) .. (233.07,140.43) ;
\draw [shift={(231.3,138.43)}, rotate = 410.19] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
...
\draw    (449.78,106.19) .. controls (481.08,80.24) and (476.55,172.53) .. (447.3,137.43) ;
\draw [shift={(447.3,108.43)}, rotate = 315.86] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
...
we obtain
\todo{Complete calculations, see also previous section. Notice that mandl use wrong normalization}
\begin{align*}
\overline{| \mathcal M_{fi}^{\textup{ext}}|}^2_{\textup{LAB}}
